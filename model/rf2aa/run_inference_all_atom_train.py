...
class ModelRunner:
    ...
    def write_outputs(self, input_feats, outputs):
        ...
        plddts = plddts[0]
        writepdb(os.path.join(f"{self.config.output_path}", f"predict_{self.config.job_name}.pdb"), 
                 xyz_allatom, 
                 seq_unmasked, 
    ...
    def fix_outputs(self):
        ...
        predict_pdb = os.path.join(f"{self.config.output_path}", f"predict_{self.config.job_name}.pdb")
        fix(
            name = name,
            clean_pdb = clean_pdb,
