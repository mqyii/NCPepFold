...
class PositionalEncoding2D(nn.Module):
    ...
    def __init__(self, d_pair, minpos=-32, maxpos=32, maxpos_atom=8, p_drop=0.15, use_same_chain=False, enable_same_chain=False, nc_cycle = False, seq_len = False): # 🌈
    ...
    #     i = torch.arange(idx.shape[1], device=seq.device)
    #     ij = torch.stack([i, i + idx.shape[1]], dim=-1)
    #     tmp_cyc_offset = torch.abs(ij[:, None, :, None] - ij[None, :, None, :])
    #     cyc_offset = tmp_cyc_offset.min(dim=3).values.min(dim=2).values
    #     offset = torch.sign(res_dist) * cyc_offset

    #     bins = torch.arange(self.minpos, self.maxpos+1, device=seq.device)
    #     ib_res = torch.bucketize(offset, bins).long() # (B, L, L)
    #     # seqsep[0] = (seqsep[0] + L//2)%L - L//2
    #     # print(ib_res) # 🌈
    #     ffset = self.emb_res(ib_res)
    ...
    def forward(self, seq, idx, bond_feats, dist_matrix, same_chain=None):
        ...
        # pep_path = Path(self.cfg.protein_inputs['A']['fasta_file'])
        # with open(pep_path, 'r') as f:  # 🌈🌈🌈
        #     record = list(SeqIO.parse(f, 'fasta'))
        # seq_len = len(record[0].seq)

        res_dist, atom_dist = get_res_atom_dist(idx, bond_feats, dist_matrix, sm_mask,
            minpos_res=self.minpos, maxpos_res=self.maxpos, maxpos_atom=self.maxpos_atom, nc_cycle = self.nc_cycle, seq_len = self.seq_len)  # 🌈
        # print(f"res_dist.shape: {res_dist.shape}")
        # with open("/home/light/mqy/m_Transformer/data/333.txt","w") as f:
        #     f.write(f"{res_dist.detach().cpu().numpy().astype(int).tolist()}\n")
        # exit()
        # c1 = [2, 4, 6]
        # c2 = [15, 13, 11]
        # c1 = [2]
...
class Str2Str(nn.Module):
    def __init__(self, d_msa=256, d_pair=128, d_state=16, d_rbf=64,
            ...
            nextra_l0=0, nextra_l1=0, p_drop=0.1, nc_cycle = False, seq_len = False
    ):  #🌈🌈
    ...
    def forward(self, msa, pair, xyz, state, idx, rotation_mask, bond_feats, dist_matrix, atom_frames, is_motif, extra_l0=None, extra_l1=None, use_atom_frames=True, top_k=128, eps=1e-5):
        ...
        #     record = list(SeqIO.parse(f, 'fasta'))
        # seq_len = len(record[0].seq)
        # nc_cycle = self.cfg.cyclize # 🌈
        neighbor = get_seqsep_protein_sm(idx, bond_feats, dist_matrix, rotation_mask, nc_cycle = self.nc_cycle, seq_len= self.seq_len) # 🌈seq_len 
...
class IterBlock(nn.Module):
    def __init__(self, d_msa=256, d_pair=128, d_rbf=64,
                 ...
                 fit=False, tscale=1.0, nc_cycle = False, seq_len = False
                 ):
