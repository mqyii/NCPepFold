...
from rf2aa.ss_combination import ss_generation
from rf2aa.model.RoseTTAFoldModel import RoseTTAFoldModule
from rf2aa.training.recycling import recycle_step_legacy
from rf2aa.util import writepdb, is_atom, Ls_from_same_chain_2d, set_combination
...
class ModelRunner:
    ...
    def write_outputs(self, input_feats, outputs, idx=None):
        ...
        plddts = plddts[0]
        if idx:  # 🌈
            tmp_pdb_dict = f"{self.config.output_path}/tmp_{self.config.job_name}"
            pdb_path = f"{self.config.output_path}/{self.config.job_name}_{idx}.pdb"
        else:
            pdb_path = f"{self.config.output_path}/{self.config.job_name}.pdb"
    ...
    def get_combinations(self):
        fasta_info = self.config.protein_inputs
        fasta_file_path = fasta_info['A']['fasta_file']  # 需要改🌈，如果是复合物的话
    ...
    def infer(self):  # 🌈
        ...
        combinations, c_pos = self.get_combinations()  # 🎀🎀🎀🎀🎀
        ss_results = []
        if combinations:
            for i, combination in enumerate(combinations):
                # print(combination)
                input_feats_i = set_combination(input_feats, combination)
                # print(input_feats_i.bond_feats)
                outputs = self.run_model_forward(input_feats_i)
                # print(outputs)
                err_dict = self.write_outputs(input_feats_i, outputs, i+1)
                print(f"idx{i+1}'s plddt: {err_dict['mean_plddt']}")
                ss_results.append(err_dict)
            
            max_plddt_idx, max_err_dict = max(enumerate(ss_results), key=lambda x: torch.mean(x[1]['plddts'][0, c_pos]).item())
            # max_plddt_idx, max_err_dict = max(enumerate(ss_results), key=lambda x: x[1]['mean_plddt'])
            for i in range(len(ss_results)):
                if i != max_plddt_idx:
                    os.remove(f"{self.config.output_path}/{self.config.job_name}_{i+1}.pdb")
            torch.save(max_err_dict, os.path.join(f"{self.config.output_path}", 
                                          f"{self.config.job_name}_aux.pt"))
        else:
            input_feats = self.construct_features()
