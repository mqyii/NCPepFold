...
import sys
from rf2aa.ss import calc_offset_matrix
...
def get_seqsep_protein_sm(idx, bond_feats, dist_matrix, sm_mask, nc_cycle = False, seq_len = False):
    '''
    ...
    res_dist, atom_dist = get_res_atom_dist(idx, bond_feats, dist_matrix, sm_mask, nc_cycle = nc_cycle, seq_len = seq_len) #🌈 seq_len
    # np_res_dist = res_dist[0].detach().cpu().numpy()
    # int_res_dist = np.floor(np_res_dist).astype(int)
    # df = pd.DataFrame(int_res_dist)
    # df.to_csv('/home/light/mqy/ncaa/RoseTTAFold-All-Atom/tensor.csv', index=False, header=False)
    # print(res_dist)
    # exit()
    # c1 = [2, 4, 6]
    # c2 = [15, 13, 11]
    # # c1 = [2]
    # # c2 = [10]
    # res_dist = calc_offset_matrix(res_dist,c1, c2)
    # L = idx.shape[1]  # 🌈
    # res_dist[0] = (res_dist[0] + L//2)%L - L//2  # 🌈
...
def get_res_atom_dist(idx, bond_feats, dist_matrix, sm_mask, minpos_res=-32, maxpos_res=32, maxpos_atom=8, nc_cycle = False, seq_len = False): # 🌈 seq_len
    '''
    ...
    config = ConfigPro.load_config()
    # if config is not None:
    #     chain_num = 0
    #     if config.protein_inputs is not None:
    #         for _ in config.protein_inputs:
    #             chain_num += 1
    #         if chain_num != 1:
    #             complex = True
    #         else:
    #             complex = False
    #     else:
    #         print("config load unsuccessfully!")
    ...
    if nc_cycle:
        print("it is a nc peptide")
        # print("nc_cycle yes")
        idx_pep = idx[0][idx[0] < 21]
        if complex:
            print("it is a nc peptide complex")
            idx_pep = idx[0][idx[0] < 21]
            aa_num = len(idx_pep)
            # idx_pep = torch.arange(seq_len).unsqueeze(0)
            # res_dist_pep = torch.clamp(idx_pep[0,None,:] - idx_pep[0,:,None],
            #                    min=minpos_res, max=maxpos_res)
            res_dist_pep = torch.clamp(idx_pep[None,:] - idx_pep[:,None],
                               min=minpos_res, max=maxpos_res)
            res_dist_pep = (res_dist_pep + seq_len//2)%seq_len - seq_len//2
            res_dist_prot[:aa_num, :aa_num] = res_dist_pep

            # np_res_dist_prot = res_dist_prot.detach().cpu().numpy()
            # int_res_dist_prot = np.floor(np_res_dist_prot).astype(int)
            # df = pd.DataFrame(int_res_dist_prot)
            # df.to_csv('/home/light/mqy/ncaa/RoseTTAFold-All-Atom/tensor.csv', index=False, header=False)
            # exit()
        else:
            res_dist_prot = (res_dist_prot + seq_len//2)%seq_len - seq_len//2  # 🌈
    ...
    i_sm = i_s[sm_mask[i_s]]  # [17, 19]
    i_prot = j_s[sm_mask[i_s]] # [11, 12]
