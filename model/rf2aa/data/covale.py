...
def load_covalent_molecules(protein_inputs, config, model_runner):
    ...
    covalent_bonds_list = eval(config.covale_inputs)  #🌈
    chainid_to_input_list = []  #🌈
    residues_to_atomize_list = []  #🌈
    for i, (chain, details) in enumerate(config.sm_inputs.items()):  #🌈
        sm_inputs = delete_leaving_atoms({chain: details})
        covalent_bonds = covalent_bonds_list[i]
        ...
        for chain, combined_molecule in combined_molecules.items():
            ...
            xyz = recompute_xyz_after_chirality(mol, config)
            input = compute_features_from_obmol(mol, msa, xyz, model_runner)
            chainid_to_input[chain] = input
        chainid_to_input_list.append(chainid_to_input)  #🌈
        residues_to_atomize_list.append(residues_to_atomize)  #🌈
    
    return chainid_to_input_list, residues_to_atomize_list
...
def recompute_xyz_after_chirality(obmol, config):  # flag: bool = False
    builder = openbabel.OBBuilder()
    if "builder_obmol" in config:
        pass
    else:
        builder.Build(obmol)
