...
def parse_templates_raw(ffdb, hhr_fn, atab_fn, max_templ=20):
    ...
    # process hits
    counter = 0
    xyz,qmap,mask,f0d,f1d,ids,seq,score_i = [],[],[],[],[],[],[],[] # 🌈 score_i
    for data in hits:
        if len(data)<7:
            continue
        ...
        ncol = sel1.shape[0]
        if ncol < 10:
            continue  # 🌈所以一般来讲不能预测10以下的多肽链
        score_i.append(data[3][2])  # 🌈
        ids.append(data[0])
        f0d.append(data[3])
        f1d.append(np.array(data[2])[sel1])
        xyz.append(data[4][sel2])
    ...
    return torch.from_numpy(xyz), torch.from_numpy(mask), torch.from_numpy(qmap), \
           torch.from_numpy(f0d), torch.from_numpy(f1d), torch.from_numpy(seq), ids, score_i  # 🌈
