...
def TemplFeaturize(tplt, qlen, params, offset=0, npick=1, npick_global=None, pick_top=True, same_chain=None, random_noise=5, deterministic: bool = False, chain = None):
    if deterministic:
        random.seed(0)
        np.random.seed(0)
        torch.manual_seed(0)
    ...
    if seqID_cut <= 100.0:
        ...
        tplt_valid_idx = torch.arange(len(tplt['ids']))  # 模型默认选取所有模板
    ...
    if not pick_top: # select randomly among all possible templates
        ...
        if chain == "A":
            ...
            # print(sample)
            count_50 = sum(score >= 50 for score in tplt['score'])
            # print(f"there are {count_50} MSA score >= 50")
            count_15 = sum(score >= 15 for score in tplt['score'])  # 🌈 17可能还需变一下
            # print(f"there are {count_17} MSA score >= 17")
            if 0 < count_50 < 4:
                sample = torch.arange(count_50)
                # print(f"count_50 is {count_50}")
            if count_15 == 0:  # 🌈 17可能还需变一下
                # print("chain A return blank template")
                return blank_template(npick_global, qlen, random_noise)
        ...
        # sample = torch.arange(4,8)  # 🌈
        # sample = torch.randperm(min(50,ntplt))[:npick]  # 🌈 原  2lyf:20个里面随机选4个,4个数量可能也需要变一下
