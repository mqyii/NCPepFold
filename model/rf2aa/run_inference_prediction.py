...
class ModelRunner:
    ...
    def __init__(self, config) -> None:
        ...
        self.seq_len = self.pep_length() #xxxx
    ...
    def load_model(self):
        ...
        checkpoint = torch.load("/home/light/mqy/ncaa/notebooks/tmp/ckpt/rfaa_20241019010809_84.pt", map_location=self.device)
        # print("/home/light/mqy/ncaa/notebooks/tmp/ckpt/rfaa_20241019010809_84.pt")
    ...
    def write_outputs(self, input_feats, outputs):
        ...
        plddts = plddts[0]
        writepdb(os.path.join(f"{self.config.output_path}", f"finetune_predict_{self.config.job_name}.pdb"), 
                 xyz_allatom, 
                 seq_unmasked, 
        ...
        torch.save(err_dict, os.path.join(f"{self.config.output_path}", 
                                          f"{self.config.job_name}_aux_finetune.pt"))
    def fix_outputs(self):
        ...
        predict_pdb = os.path.join(f"{self.config.output_path}", f"finetune_predict_{self.config.job_name}.pdb")
        fix(
            name = name,
            clean_pdb = clean_pdb,
    ...
    def prediction_infer(self):
        ...
        with open(f'/home/light/mqy/ncaa/notebooks/tmp/loss_txt/FAPEloss_{runner_version}_ss.txt', 'a') as f:
        ...
        loader = data_loader('/home/light/mqy/ncaa/notebooks/tmp/train_ss.txt', batch_size=1)
