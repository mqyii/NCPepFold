...
def get_protein_bond_feats(protein_L, nc_cycle = False):
...
def set_combination(input_feats, combination):  # # 🌈# 🌈# 🌈# 🌈# 🌈
    """
    combination = [[0, 16], [4 ,18], [9, 23]]
    bond_feats[0,16] = 5
    bond_feats[16,0] = 5
    bond_feats[4,18] = 5
    bond_feats[18,4] = 5
    ...
    bond_feats[23,9] = 5
    """
    _input_feats = deepcopy(input_feats)
    for ss in combination:
        _input_feats.bond_feats[ss[0], ss[1]] = 5        
        _input_feats.bond_feats[ss[1], ss[0]] = 5
    return _input_feats
...
class ConfigPro:
    _config_file = os.path.join('/tmp/.config.json')

    @classmethod
    def save_config(self, config):
        """Save the configuration to a JSON file."""
        joblib.dump(config, self._config_file)

    @classmethod
    def load_config(self):
        """Load the configuration from a JSON file."""
        if os.path.exists(self._config_file):
            config = joblib.load(self._config_file)
            return config
        else:
            return None